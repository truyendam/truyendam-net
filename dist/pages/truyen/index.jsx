...
function AllStoriesPage() {
    ...
    const sortedStories = [...stories].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
    ...
    return (<>
      <head_1.default>
        ...
        <meta name="description" content="Danh sách truyện sex 18+ hay nhất Việt Nam. Đọc truyện người lớn, truyện ngoại tình, truyện gợi cảm miễn phí – cập nhật liên tục tại Truyendam.net!"/>
      ...
      <div className="min-h-screen bg-black text-white px-4 py-10">
        ...
        <div className="max-w-6xl mx-auto">
          <div ref={resultsRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            var _a;
            return (<link_1.default key={story.slug} href={`/truyen/${story.slug}`} className="group relative bg-zinc-900 rounded-2xl shadow hover:shadow-pink-500/30 hover:scale-105 transition-all flex flex-col">
                  <div className="w-full aspect-[3/4] rounded-t-2xl overflow-hidden">
                    <image_1.default src={story.coverImage || "/default-cover.jpg"} alt={story.title} width={320} height={420} className="object-cover w-full h-full group-hover:scale-110 transition" sizes="320px" priority/>
                  </div>
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1 mt-auto text-xs">
                      {(_a = story.tags) === null || _a === void 0 ? void 0 : _a.map((tag) => (<span key={tag} className="bg-zinc-700 text-pink-200 px-2 py-[2px] rounded mr-1">{tag}</span>))}
                    </div>
